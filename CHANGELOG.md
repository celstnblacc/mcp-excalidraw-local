...
- Docker image refs corrected again: `artificemachine` is the GitHub org but not the Docker Hub namespace — published images live under `newblacc` (`newblacc/excalidraw-mcp-sentinel`, `newblacc/excalidraw-mcp-sentinel-canvas`). Updated `docker-compose.yml`, `.github/workflows/{docker,release}.yml`, and `README.md` docker pull/run instructions accordingly. GitHub source-URL references (Dockerfile OCI labels, package.json repo URLs) correctly stay `artificemachine`.
- `v1.2.1` published to npm (registry) and Docker Hub (`newblacc/excalidraw-mcp-sentinel:v1.2.1`/`latest`, `newblacc/excalidraw-mcp-sentinel-canvas:v1.2.1`/`latest`) manually from local machine — `release.yml` CI automation has broken action pins (`actions/create-github-app-token`, `softprops/action-gh-release` SHAs no longer resolve) and the repo has no `NPM_TOKEN`/`DOCKERHUB_*`/`APP_ID`/`APP_PRIVATE_KEY` secrets configured, so the automated pipeline can't run end-to-end yet
- `release.yml`'s "Version bump & release" job dropped the `actions/create-github-app-token` step (its pinned SHA no longer resolves, and a custom GitHub App bot identity is unnecessary complexity for a solo-maintainer repo) — now commits/tags/pushes/creates the release using the workflow's own `GITHUB_TOKEN` (`contents: write` already granted). Also repinned `softprops/action-gh-release` from a dead SHA to v3.0.1 (`718ea10b132b3b2eba29c1007bb80653f286566b`). `NPM_TOKEN` and `DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` repo secrets still need to be added before the publish jobs can run in CI.
